...
public class FileParser {
	public List<GameObject> parseEnnemies(Room room, string levelType, string roomType, string roomId, Transform decorContainer, Transform ennemyContainer, Transform floorTransform) {
		...
		for(int i=0; i<lines.Length; i++) {
			...
			if(elements[0] == "ennemy") {
				...
				Vector3 position = new Vector3( float.Parse(elements[2]), float.Parse(elements[3]), float.Parse(elements[4]));
				...
				ennemyGO.GetComponent<Ennemy>().room = room;
				gos.Add(ennemyGO);
			}
		}			
		return gos;
